...
namespace RoutePlanning
{
    internal static class Program
    {
        ...
        {
            foreach (string testFile in Directory.GetFiles("tests", difficulty + "-*.txt"))
            {
                string[] lines = File.ReadAllLines(testFile);
                var answer = double.Parse(lines.First(), CultureInfo.InvariantCulture); //ожидаемая длинна
                Point[] ps = lines.Skip(1).Select(line => line.Split().Select(int.Parse).ToArray()) //преобразование текстового файла в массив Point[]
                    .Select(cs => new Point(cs[0], cs[1])).ToArray();
                var sw = Stopwatch.StartNew();
                var order = solve(ps);
                sw.Stop();
                var len = ps.GetPathLength(order); //вызов метода расширения на Point[] ps для вычисления расстояния
                var testName = Path.GetFileNameWithoutExtension(testFile); // присваивание имени теста
                var isPassed = IsPassed(len, order, answer, ps.Length, pathsMustMatch); //проверка вычисленного расстояния и ожидаемого расстояния
                LogTest(testName, ps.Length, len, sw.Elapsed, answer, isPassed); //вывод результатов теста
